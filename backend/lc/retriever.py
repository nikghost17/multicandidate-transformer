...
def search_candidates(
    ...
    section_type : str   — optional section filter (e.g. "skills", "experience")

    Returns
    -------
    list[dict]  with keys: candidate_id, text, similarity, section_type, section_title
    """
    from lc.mongo_vectorstore import search

    hits = search(query, top_k=top_k * 2, section_type=section_type)  # over-fetch then deduplicate
